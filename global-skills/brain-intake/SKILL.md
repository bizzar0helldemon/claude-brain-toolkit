...
- **Identity template:** `{{SET_YOUR_BRAIN_PATH}}/IDENTITY.template.md` (used to seed `IDENTITY.md` on first run)
- **Intake sessions:** `{{SET_YOUR_BRAIN_PATH}}/intake/sessions/`
- **Intake index:** `{{SET_YOUR_BRAIN_PATH}}/intake/_INDEX.md`
- **Templates:** `{{SET_YOUR_BRAIN_PATH}}/brain-scan-templates.md`

All vault paths referenced below (`IDENTITY.md`, `intake/`, `creative/`, `projects/`, `CLAUDE.md`) are relative to the brain root above.
...
**First, ensure `IDENTITY.md` exists.** If `{{SET_YOUR_BRAIN_PATH}}/IDENTITY.md` is missing, seed it from the template before writing:

```bash
[ -f "$BRAIN_PATH/IDENTITY.md" ] || cp "$BRAIN_PATH/IDENTITY.template.md" "$BRAIN_PATH/IDENTITY.md"
```

If neither the file nor the template exists, create `IDENTITY.md` with the standard sections (Life Story, Career History, Creative Voice, Values & Worldview, Communication Preferences).

Then process the captured information into the appropriate structured files:
