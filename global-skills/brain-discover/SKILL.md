...
- **Creative content:** `{{SET_YOUR_BRAIN_PATH}}/creative/`
- **Discovery logs:** `{{SET_YOUR_BRAIN_PATH}}/intake/discoveries/`
- **Identity profile:** `{{SET_YOUR_BRAIN_PATH}}/IDENTITY.md`
- **Project index:** `{{SET_YOUR_BRAIN_PATH}}/projects/_INDEX.md`

All vault destination paths below are relative to the brain root above. The *scan path* (`$ARGUMENTS`) is an external directory and is read-only — never modified.
