...
- **Drop zone:** `{{SET_YOUR_BRAIN_PATH}}/inbox/`
- **Templates:** `{{SET_YOUR_BRAIN_PATH}}/brain-scan-templates.md`

Determine the brain vault root by checking (in order):
1. The `$BRAIN_PATH` environment variable
2. The current working directory (if it contains `CLAUDE.md` and `brain-scan-templates.md`)
3. The `OBSIDIAN_VAULT` environment variable (legacy fallback)
4. Ask the user

Read `brain-scan-templates.md` from the vault root for canonical template definitions before processing any files. All destination paths below are relative to the brain root.
